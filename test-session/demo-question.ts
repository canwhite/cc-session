...
async function demoQuestionMode() {
  ...
  console.log(
    "- System prompt: Focus on answering questions and providing information. Can read files and search codebases, but cannot modify files or execute system commands."
  );
  console.log("\n");
  ...
  const unsubscribe = session.subscribe((currentSession, broadcastMessage) => {
    switch (broadcastMessage.type) {
      case "session_info":
        console.log(
          `Session info - Message count: ${broadcastMessage.messageCount}, Active: ${broadcastMessage.isActive}`
        );
        break;
        ...
        if (message.content && message.content.length > 0) {
          ...
          message.content.forEach((part, index) => {
            ...
            if (part.toolResult) {
              console.log(
                `     Tool result: ${
                  part.toolResult.is_error ? "Error" : "Success"
                } - ${
      ...
      case "tool_result_updated":
        ...
        console.log(
          `Result: ${broadcastMessage.result.is_error ? "Error" : "Success"}`
        );
        break;
  ...
  try {
    ...
    console.log(
      `Last modified: ${new Date(session.lastModifiedTime).toISOString()}`
    );
    ...
    console.log(
      `Token usage: total=${usage.totalTokens}, cost=$${usage.totalCost.toFixed(
        6
